...
export default function SearchBar({handleSearch, value} : {handleSearch: (string) => void, value: string}) : ReactElement{
    ...
    const handleAutoComplete = async (): Promise<void> => {
        ...
        setLoading(true)
        const res = await getAutoComplete(search)
    ...
    const instantlySetSearch = async (query: string): Promise<void> => {
        setSearch(query)
        setDebounceValue(0)
    }
                    ...
                    <div className={style.filter__searchBar__autocomplete}>
                        <ul>
                            ...
                            {!loading && autocomplete.map((item: string, index: number) => {
                                return <li key={index} onClick={() => handleAutoCompleteClick(item)} className={style.filter__searchBar__autocomplete__item}>{item}</li>
                            })}
